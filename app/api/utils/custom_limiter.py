...
class ActionLimiter:
    ...
    `blockInterval` - интервал блокировки, в течение которого операция будет заблокирована после достижения максимального количества попыток.

    `resetInterval` - интервал сброса, после истечения счетчик попыток будет снова начат с нуля.
    """
